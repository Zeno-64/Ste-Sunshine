...
export function IdeasGrid() {
  return (
    <section id="ideias" className="py-24 md:py-32">
      <div className="container-full">
        <Reveal className="text-center mb-16">
          <p className="text-eyebrow text-primary mb-3">Inspire-se</p>
          <h2 className="font-serif text-4xl md:text-5xl text-foreground">
            O que vira ímã
          </h2>
          <p className="mt-4 text-muted-foreground max-w-lg mx-auto">
            Qualquer memória cabe aqui. Estas são as mais pedidas — clique na que
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6">
          {occasions.map((occasion, index) => (
            <div key={occasion.slug} className={spanClass[occasion.span]}>
              <Reveal delay={index * 0.08}>
                <a {...linkProps(occasion.href)} className="group block relative">
                  <div
                    className={cn(
                      "relative overflow-hidden bg-muted/50",
                      ratioClass[occasion.ratio],
                    )}
                  >
                    <img
                      src={occasion.image}
